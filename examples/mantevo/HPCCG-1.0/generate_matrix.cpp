...

{
  ...
  *xexact = new double[local_nrow];
  ...
  (*A)->dyn_size = sizeof(char) +
                   local_nrow * sizeof(int) +
                   local_nrow * sizeof(double*) * 2 +
                   local_nrow * sizeof(int*) +
                   local_nrow * sizeof(double) * 2; // Note: xexact not used in kernel!

  // Allocate arrays that are of length local_nnz
  (*A)->list_of_vals = new double[local_nnz];
  (*A)->list_of_inds = new int   [local_nnz];

  (*A)->dyn_size += local_nnz * (sizeof(double) + sizeof(int));
