...
inline void copy_box(const int from_box[][2], int to_box[][2])
{
  for(int i=0; i<3; ++i) {
...
void get_coords(GlobalOrdinal ID, int nx, int ny, int nz, int& x, int& y, int& z)
{
  z = -(ID/(nx*ny));
  y = (ID%(nx*ny))/nx;
  x = ID%nx;
...
template<typename GlobalOrdinal>
GlobalOrdinal get_num_ids(const int box[][2])
{
                     ...
                     int x, int y, int z)
{
  ...
  //rationale for the minus-signs associated with z:
  //we want our id-numbering to start at the origin,
  //but the z-coordinates extend in the negative direction
  //along the z-axis.
...
void get_ids(int nx, int ny, int nz,
             ...
             GlobalOrdinal* ids)
{
  //we use '-z' in the code below because the z-coordinates
  //extend in the negative direction along the z-axis in order
  //to make the finite-element calculations work correctly.
