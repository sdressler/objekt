...
#ifdef MINIFE_HAVE_STK
#include <stk_mesh_description.hpp>
#include <stk_generate_matrix_structure.hpp>
#include <stk_assemble_FE_data.hpp>
...
void driver(const int global_box[][2], int my_box[][2], ComputeNodeType& compute_node, Parameters& params, YAML_Doc& ydoc) {
  ...
  //Note that 'stk_mesh_description' contains a fully populated stk::mesh
  //database, while 'simple_mesh_description' is a virtual or conceptual
...
#ifdef MINIFE_HAVE_STK
  stk::ParallelMachine comm = stk::parallel_machine_null();
#ifdef HAVE_MPI
  comm = MPI_COMM_WORLD;
#endif
  bool create_elem_mat_fields = params.use_elem_mat_fields==1;
  stk_mesh_description<Scalar,GlobalOrdinal> mesh(comm, create_elem_mat_fields);
  populate_mesh( mesh , global_box , my_box );
#else
 ...
 //Declare matrix object:
  ...
  SparseMatrix<Scalar,LocalOrdinal,GlobalOrdinal,ComputeNodeType> A(compute_node);
...
//  //For large systems it seems to be necessary to add a value to the diagonal
//  //in order to get the CG solve to converge. Should this be necessary?
//  //Does it indicate a problem with the dirichlet boundary-condition that is
//  //applied below?
//  Scalar value = 1.e-8*global_nx;
//  value *= global_ny*global_nz;
//  value += 0.5;
//
//#ifdef MINIFE_DEBUG
//  outstream() << "add to diag: " << value << std::endl;
//#endif
//
//  add_to_diagonal(value, A);

  //Now apply a dirichlet boundary-condition to the x=0 face of the global box.
  //(This is needed to make the matrix non-singular.)
  ...
  RUN_TIMED_FUNCTION("imposing Dirichlet BC...",
            impose_dirichlet_x0(1.0, A, b, global_nx+1, global_ny+1, global_nz+1, mesh),
                     dirbc_time, t_total);
  ...
  typedef SparseMatrix<Scalar,LocalOrdinal,GlobalOrdinal,ComputeNodeType> MatrixType;

  t_total = mytimer() - t_start;

//  MatrixType A2(compute_node);
//  copy_matrix(A, A2);
