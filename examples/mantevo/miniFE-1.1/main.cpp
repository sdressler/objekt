...
inline void print_box(int myproc, const char* name, const int box[][2],
                      const char* name2, const int box2[][2])
{
...
int main(int argc, char** argv) {
  ...
  int global_nx = params.nx, global_ny = params.ny, global_nz = params.nz;

  int global_box[3][2] = { { 0, global_nx }, { 0, global_ny }, { 0, global_nz } };
  int (*local_boxes)[3][2] = (int(*)[3][2])std::malloc(sizeof(int)*numprocs*3*2);


  box_partition(0, numprocs, 2, global_box, local_boxes);

  //my_box = local_boxes[myproc]
  int (*const my_box)[2] = (int(*)[2])local_boxes[myproc];
...
//print_box(myproc, "global-box", global_box, "local-box", my_box);
  ...
  YAML_Doc doc("miniFE", MINIFE_VERSION, ".", osstr.str());  if (myproc == 0) {
  ...
  std::cout << "Size: " << sizeof(int) * 3 * 2 + sizeof(int) * 2 + params.size() + doc.size();
  std::cout << " Bytes\n";
