...
make_local_matrix(MatrixType& A)
{
  ...
  GlobalOrdinal* Acols = local_nrow>0 ? &(A.packed_cols[0]) : NULL;
  std::vector<GlobalOrdinal>& external_index = A.external_index;

  if (A.row_offsets.size() > 0) {
  for(size_t i=0; i<A.row_offsets.size()-1; ++i) {
    for(LocalOrdinal j=A.row_offsets[i]; j<A.row_offsets[i+1]; ++j) {
