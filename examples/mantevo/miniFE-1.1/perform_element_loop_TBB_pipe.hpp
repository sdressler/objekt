...
private:
  /** This operator takes a vector of elem-data objects which have an
    * element-diffusion-matrix and source-vector, and assembles each row
    * into the linear-system, after locking that row to make sure no
    * other thread is assembling the same global row at the same time.
    */
  void* operator()(void* item) {
    if (item == NULL) return NULL;
    std::vector<ElemData<GlobalOrdinal,Scalar> >* elemdata_vec = static_cast<std::vector<ElemData<GlobalOrdinal,Scalar> >*>(item);
    ...
    for(size_t e=0; e<elemdata_vec->size(); ++e) {
      ...
      for(size_t i=0; i<nnodes; ++i) {
        ...
        A_.sum_in(row, nnodes, elemdata.elem_node_ids,
                  &(elemdata.elem_diffusion_matrix[i*nnodes]));
