...
UTEST_CASE(get_num_ids)
{
  int box0[3][2] = {{ 0, 2 }, { 0, 2 }, { 0, 1 } };
  if (miniFE::get_num_ids<int>(box0) != 4) {
    return false;
  }

#ifndef MINIFE_NO_LONG_LONG
  int box1[3][2] = {{ 0, 2000 }, { 0, 2000 }, { 0, 2000 } };
  if (miniFE::get_num_ids<long long>(box1) != 8000000000) {
    return false;
  }
#endif
...
UTEST_CASE(get_id)
{
  ...
  int ret = 0;
  ret += check_get_id<int>(nx,ny,nz, 0,0,0,   0, "utest1-0");
  ret += check_get_id<int>(nx,ny,nz, 1,0,0,   1, "utest1-1");
  ret += check_get_id<int>(nx,ny,nz, 0,1,0,   2, "utest1-2");
  ret += check_get_id<int>(nx,ny,nz, 1,1,0,   3, "utest1-3");
  ret += check_get_id<int>(nx,ny,nz, 1,1,-1,  7, "utest1-4");
  ret += check_get_id<int>(nx,ny,nz, 1,0,-1,  5, "utest1-5");
  ret += check_get_id<int>(nx,ny,nz, 0,0,-1,  4, "utest1-6");
  ret += check_get_id<int>(nx,ny,nz, 2,0,-1, -1, "utest1-7");

#ifndef MINIFE_NO_LONG_LONG
  nx=2000; ny=2000; nz=2000;

  ret += check_get_id<long long>(nx,ny,nz, 0,0,0,  0, "utest1-8");
  ret += check_get_id<long long>(nx,ny,nz, 1,0,0,  1, "utest1-9");
  ret += check_get_id<long long>(nx,ny,nz, 0,0,-1,  4000000, "utest1-10");
  ret += check_get_id<long long>(nx,ny,nz, 1999,1999,-1999,  7999999999, "utest1-10");
#endif
...
UTEST_CASE(get_ids)
{
  int box[3][2] = {{ 0, 2 }, { 0, 2 }, { 0, 1 } };
  std::vector<int> ids(4);
  ids[0] = 0; ids[1] = 1; ids[2] = 2; ids[3] = 3;
  std::vector<int> ids2(4);
  miniFE::get_ids(2, 2, 1, box, &ids2[0]);
  if (ids != ids2) {
    return false;
  }

#ifndef MINIFE_NO_LONG_LONG
  int box2[3][2] = {{1998,2000}, {1998,2000}, {1998,2000}};
  std::vector<long long> ids3(8);
  ids3[0] = 7995997998;
  ids3[1] = 7995997999;
  ids3[2] = 7995999998;
  ids3[3] = 7995999999;
  ...
  std::vector<long long> ids4(8);
  miniFE::get_ids(2000,2000,2000, box2, &ids4[0]);
  if (ids3 != ids4) {
    return false;
  }
#endif
...
UTEST_CASE(generate_matrix_structure1)
{
  ...
  miniFE::SparseMatrix<Scalar, int, int, SerialComputeNode> A(compute_node);
...
UTEST_CASE(get_hex8_node_coords_3d)
{
  ...
  miniFE::get_hex8_node_coords_3d(0, 0, 0, &testcoords[0]);
...
//unintended changes as I continue working on the code for various reasons.
  ...
  elem_mat.resize(64);
  elem_mat[0]  =  0.333333333169113;
  elem_mat[1]  =  -0.166666666611927;
  elem_mat[2]  =  0.166666666611927;
  elem_mat[3]  =  0.166666666830887;
  elem_mat[4]  =  -0.166666666611927;
  elem_mat[5]  =  -0.49999999994526;
  elem_mat[6]  =  -5.47402297101929e-11;
  elem_mat[7]  =  0.166666666611927;
  elem_mat[8]  =  -0.166666666611927;
  elem_mat[9]  =  0.333333333169113;
  elem_mat[10] =  -0.166666666611927;
  elem_mat[11] =  -0.49999999994526;
  elem_mat[12] =  0.166666666611927;
  elem_mat[13] =  0.166666666830887;
  elem_mat[14] =  0.166666666611927;
  elem_mat[15] =  -5.4740235781725e-11;
  elem_mat[16] =  0.166666666611927;
  elem_mat[17] =  -0.166666666611927;
  elem_mat[18] =  0.333333333169113;
  elem_mat[19] =  0.166666666830888;
  elem_mat[20] =  -5.47402314449164e-11;
  elem_mat[21] =  -0.49999999994526;
  elem_mat[22] =  -0.166666666611927;
  elem_mat[23] =  0.166666666611927;
  elem_mat[24] =  0.166666666830887;
  elem_mat[25] =  -0.49999999994526;
  elem_mat[26] =  0.166666666830887;
  elem_mat[27] =  1.66666666628349;
  elem_mat[28] =  -0.49999999994526;
  elem_mat[29] =  -0.666666666940368;
  elem_mat[30] =  -0.49999999994526;
  elem_mat[31] =  0.166666666830887;
  elem_mat[32] =  -0.166666666611927;
  elem_mat[33] =  0.166666666611927;
  elem_mat[34] =  -5.47402331796398e-11;
  elem_mat[35] =  -0.49999999994526;
  elem_mat[36] =  0.333333333169113;
  elem_mat[37] =  0.166666666830888;
  elem_mat[38] =  0.166666666611926;
  elem_mat[39] =  -0.166666666611927;
  elem_mat[40] =  -0.49999999994526;
  elem_mat[41] =  0.166666666830888;
  elem_mat[42] =  -0.49999999994526;
  elem_mat[43] =  -0.666666666940368;
  elem_mat[44] =  0.166666666830887;
  elem_mat[45] =  1.66666666628349;
  elem_mat[46] =  0.166666666830887;
  elem_mat[47] =  -0.49999999994526;
  elem_mat[48] =  -5.47402435879807e-11;
  elem_mat[49] =  0.166666666611927;
  elem_mat[50] =  -0.166666666611927;
  elem_mat[51] =  -0.49999999994526;
  elem_mat[52] =  0.166666666611927;
  elem_mat[53] =  0.166666666830887;
  elem_mat[54] =  0.333333333169113;
  elem_mat[55] =  -0.166666666611926;
  elem_mat[56] =  0.166666666611927;
  elem_mat[57] =  -5.4740235781725e-11;
  elem_mat[58] =  0.166666666611927;
  elem_mat[59] =  0.166666666830887;
  elem_mat[60] =  -0.166666666611927;
  elem_mat[61] =  -0.49999999994526;
  elem_mat[62] =  -0.166666666611927;
  elem_mat[63] =  0.333333333169113;
}
...
UTEST_CASE(diffusionMatrix)
{
  ...
  miniFE::Hex8::diffusionMatrix(testcoords, elem_mat);
...
{
  SerialComputeNode compute_node;
  miniFE::SparseMatrix<Scalar,LocalOrdinal,GlobalOrdinal,SerialComputeNode> A(compute_node);
  A.rows.resize(1,0);
...
UTEST_CASE(assemble_FE_data)
{
  ...
  std::vector<Scalar> elem_mat_correct;
  get_test_elem_mat(elem_mat_correct);

  std::vector<int> elem_node_ids(8);
  elem_node_ids[0] = 0;
  elem_node_ids[1] = 1;
  elem_node_ids[2] = 5;
  elem_node_ids[3] = 4;
  ...
  std::vector<Scalar> elem_mat_reordered(64);
  int offset = 0;
  int row = 0;
  for(int i=0; i<8; ++i) {
    row = 8*elem_node_ids[i];
    for(int j=0; j<8; ++j) {
      elem_mat_reordered[row+elem_node_ids[j]] = elem_mat_correct[offset+j];
    }
    offset += 8;
  }
...
UTEST_CASE(ser_matvec1)
{
  ...
  miniFE::SparseMatrix<Scalar,LocalOrdinal,GlobalOrdinal,ComputeNodeType> A(compute_node);
